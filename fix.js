...
export default function ThemeToggle({ className = '' }) {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={\`flex items-center justify-center w-10 h-10 rounded-full bg-[var(--bg-elevated)] border border-[var(--border-subtle)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)] transition-all shadow-sm \${className}\`}
...
fs.writeFileSync('c:/Users/Riya/OneDrive/Desktop/nirma/ARIA/aria-interviewer/frontend/src/components/ThemeToggle.jsx', code, 'utf8');const fs = require('fs');
const code = \import { useTheme } from '../context/ThemeContext';
